...
class Brain:
    ...
    def get_unencrypted_secrets(self) -> List[Dict]:
        ...
        Returns:
            List of dicts with id, agent_id, and content preview for unencrypted secrets
        """
        with self._get_cursor() as cursor:
            if self.backend == "sqlite":
                cursor.execute("""
                    ...
                    FROM memories 
                    WHERE memory_type = 'secret' AND content_encrypted = 0
                """)
            else:
                cursor.execute("""
                    SELECT id, agent_id, content, created_at 
                    FROM memories 
                    WHERE memory_type = 'secret' AND content_encrypted = FALSE
                """)
            ...
            for row in rows:
                results.append({
                    "id": row[0],
                    "agent_id": row[1],
                    "content_preview": row[2][:50] + "..." if len(row[2]) > 50 else row[2],
                    "created_at": row[3]
                })
            return results
    ...
    def migrate_secrets(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        Returns:
            Dict with migration results: count, success, failed, errors
        """
        if not self._encryption_key:
            return {
                "success": False,
                "error": "No encryption key configured. Run 'clawbrain setup' first.",
                "count": 0,
                "migrated": 0,
                "failed": 0
            }
        ...
        with self._get_cursor() as cursor:
            for secret in unencrypted:
                try:
                    # Fetch the full content
                    if self.backend == "sqlite":
                        cursor.execute("SELECT content FROM memories WHERE id = ?", (secret["id"],))
                    else:
                        cursor.execute("SELECT content FROM memories WHERE id = %s", (secret["id"],))
                    
                    row = cursor.fetchone()
                    if row:
                        original_content = row[0]
                        encrypted_content = self._encrypt(original_content)
                        
                        # Update with encrypted content
                        if self.backend == "sqlite":
                            cursor.execute("""
                                UPDATE memories 
                                SET content = ?, content_encrypted = 1 
                                WHERE id = ?
                            """, (encrypted_content, secret["id"]))
                        else:
                            cursor.execute("""
                                UPDATE memories 
                                SET content = %s, content_encrypted = TRUE 
                                WHERE id = %s
                            """, (encrypted_content, secret["id"]))
                    ...
                    failed += 1
                    errors.append({"id": secret["id"], "error": str(e)})
    ...
    def _run_auto_migration(self):
        ...
        try:
            ...
            if result.get("failed", 0) > 0:
                logger.error(f"Auto-migration had {result['failed']} failures: {result.get('errors')}")
        except Exception as e:
